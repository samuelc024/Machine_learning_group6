...
```bash
pip install torch gymnasium ale-py numpy tqdm
```

Requires **Python >= 3.10** and a CUDA-capable GPU (CPU training is supported but slow).

### 3) Train the PPO agent (best configuration)

```bash
python venture.py --train --steps 500000 --seed 42 --horizon 2048
```

To train on a second seed for variance estimation:

```bash
python venture.py --train --steps 500000 --seed 7 --horizon 2048
```

### 4) Watch the trained agent play

```bash
python venture.py --watch --seed 42 --episodes 5
```

To specify a custom model path:

```bash
python venture.py --watch --model models/seed_42/best_model.pt --episodes 3
```
...
- `numpy`
- `tqdm`
- `torch` (TensorBoard optional via `torch.utils.tensorboard`)
...
- CPU training is functional but roughly 10× slower; reduce `--steps` to 200,000 for a
  quick smoke-test.
- The ALE ROM for Venture is downloaded automatically by `gymnasium[atari]` on first run
  via the `ale-py` package.
