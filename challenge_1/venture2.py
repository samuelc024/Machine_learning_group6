...
def train_agent(
    ...
    if hparams is None:
        hparams = dict(
            env_id=ENV_ID,
            learning_rate=5e-5,          # más conservador — recompensas escasas
            buffer_size=150_000,         # buffer más grande para mayor diversidad
            learning_starts=30_000,      # más exploración inicial antes de aprender
            batch_size=64,
            gamma=0.99,
            train_freq=4,
            target_update_interval=2_000,  # sincronización menos frecuente
            exploration_fraction=0.30,   # 30% del tiempo explorando
                                        # → más tiempo descubriendo salas
            exploration_final_eps=0.05,  # 5% random final (más que Breakout)
            timesteps=timesteps,
            seed=42,
        )
